...
class Reservation {
  ...
  // (C) SAVE RESERVATION
  function save ($date, $slot, $name, $email, $tel,$pack,$guests, $notes="") {


    // (C2) DATABASE ENTRY
    try {
      $this->stmt = $this->pdo->prepare(
        "INSERT INTO `reservations` (`res_date`, `res_slot`, `res_name`, `res_email`, `res_tel`, `res_pack`,`res_guests`,`res_notes`) VALUES (?,?,?,?,?,?,?,?)"
      );
      $this->stmt->execute([$date, $slot, $name, $email, $tel, $pack, $guests, $notes]);
    } catch (Exception $ex) {
